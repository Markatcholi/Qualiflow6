...
export default function CompanyMasterDataPage() {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const { data: membership, error: membershipError } = await supabase.from("tenant_memberships").select("tenant_id,membership_role,membership_status,tenants(company_name)").eq("tenant_id", storedTenantId).ilike("user_email", userEmail).eq("membership_status", "active").maybeSingle();
  ...
  const refresh = async () => { if (tenantId) await loadAll(tenantId); };
  const insertSimple = async (table: string, code: string, label: string, reset: () => void) => { if (!code.trim() || !label.trim()) return setMessage("Code and label are required."); const { error } = await supabase.from(table).insert({ tenant_id: tenantId, code: code.trim(), label: label.trim() }); if (error) return setMessage(error.message); reset(); setMessage("Master Data updated successfully."); await refresh(); };
  const deleteRow = async (table: string, id: string) => { if (!window.confirm("Delete this Company Account Master Data item?")) return; const { error } = await supabase.from(table).delete().eq("id", id).eq("tenant_id", tenantId); if (error) return setMessage(error.message); setMessage("Master Data updated successfully."); await refresh(); };
  const insertProductPart = async () => { const partNumber = newPartCode.trim(); const partDescription = newPartDescription.trim(); if (!partNumber || !partDescription) return setMessage("Part Number and Part Description are required."); const { error } = await supabase.from("md_product_part_numbers").insert({ tenant_id: tenantId, code: partNumber, label: partNumber, part_description: partDescription, is_active: newPartIsActive }); if (error) return setMessage(error.message); setNewPartCode(""); setNewPartDescription(""); setNewPartIsActive(true); setMessage("Product Part Master updated successfully."); await refresh(); };
  const updateProductPart = async (item: ProductPartItem) => { const { error } = await supabase.from("md_product_part_numbers").update({ code: item.code.trim(), label: item.code.trim(), part_description: String(item.part_description || "").trim(), is_active: item.is_active !== false }).eq("id", item.id).eq("tenant_id", tenantId); if (error) return setMessage(error.message); setMessage("Product Part Master updated successfully."); await refresh(); };
  const insertDefectSubcategory = async () => { if (!newDefectSubcategoryCategoryCode || !newDefectSubcategoryCode.trim() || !newDefectSubcategoryLabel.trim()) return setMessage("Category, code, and label are required."); const { error } = await supabase.from("md_defect_subcategories").insert({ tenant_id: tenantId, category_code: newDefectSubcategoryCategoryCode, code: newDefectSubcategoryCode.trim(), label: newDefectSubcategoryLabel.trim() }); if (error) return setMessage(error.message); setNewDefectSubcategoryCategoryCode(""); setNewDefectSubcategoryCode(""); setNewDefectSubcategoryLabel(""); setMessage("Defect Subcategories updated successfully."); await refresh(); };
  ...
  if (!authorized) return <main style={{ padding: 20, fontFamily: "Arial, sans-serif" }}><h1>Access Denied</h1><p>Only a Company Administrator for the active Company Account can access Admin Master Data.</p><p><strong>Logged-in Email:</strong> {email || "none"}</p>{message ? <p style={{ color: "#b45309" }}>{message}</p> : null}<Link href="/workspace">Back to My Workspace</Link></main>;

  return (
    <main style={{ padding: 20, fontFamily: "Arial, sans-serif" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}><div><h1 style={{ marginBottom: 8 }}>Admin Master Data</h1><p><strong>Company Account:</strong> {companyName}</p><p><strong>Company Administrator:</strong> {email}</p></div><Link href="/workspace" style={{ padding: "10px 14px", border: "1px solid #cbd5e1", borderRadius: 8, textDecoration: "none", fontWeight: 700 }}>Back to My Workspace</Link></div>
      <div style={{ padding: 14, background: "#ecfdf5", border: "1px solid #a7f3d0", borderRadius: 10, margin: "18px 0" }}>QualiSphere starter Master Data was provisioned from the controlled baseline when this Company Account was created. Changes made here belong only to <strong>{companyName}</strong>.</div>
