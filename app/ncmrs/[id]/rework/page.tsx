...
export default function NcmrDetailPage() {
  ...
  const [reworkAssignmentFile, setReworkAssignmentFile] = useState<File | null>(null);
...
Governance override justification for opening CAPA: ${governanceOverrideJustification}`
  ...
  const uploadReworkAssignmentAttachment = async (taskId: string) => {
    if (!reworkAssignmentFile) return [];
    const safeName = reworkAssignmentFile.name.trim().replace(/[^a-zA-Z0-9._-]+/g, "_").replace(/_+/g, "_");
    const storagePath = `ncmrs/${id}/rework-tasks/${taskId}/assignment/${Date.now()}_${safeName}`;
    const upload = await supabase.storage.from("evidence").upload(storagePath, reworkAssignmentFile, { upsert: false, contentType: reworkAssignmentFile.type || undefined });
    if (upload.error) throw new Error(`Unable to upload ${reworkAssignmentFile.name}: ${upload.error.message}`);
    const publicUrl = supabase.storage.from("evidence").getPublicUrl(storagePath).data.publicUrl;
    return [{ name: reworkAssignmentFile.name, url: publicUrl, storage_path: storagePath, uploaded_at: new Date().toISOString(), uploaded_by: userEmail }];
  };
  ...
  const submitReworkTask = async () => {
    ...
    if (insertedTasks && insertedTasks.length > 0) {
      const task = insertedTasks[0];
      try {
        const assignmentAttachments = await uploadReworkAssignmentAttachment(task.id);
        if (assignmentAttachments.length > 0) {
          const { error: attachmentError } = await supabase.from("approval_tasks").update({ assignment_attachments: assignmentAttachments }).eq("id", task.id);
          if (attachmentError) throw new Error(attachmentError.message);
        }
      } catch (attachmentError: any) {
        await supabase.from("approval_tasks").delete().eq("id", task.id).eq("status", "pending");
        alert(attachmentError.message || "Unable to attach the Rework assignment evidence.");
        return;
      }
    ...
    setReworkAssignmentFile(null);
    fetchReworkTasks();
  };

  const reassignReworkTaskFromNcmr = async (task: any) => {
    if (record?.is_locked) return alert("Closed NCMRs cannot reassign Rework tasks.");
    const currentUser = normalizeApproverEmail(userEmail);
    const owner = normalizeApproverEmail(record?.owner || record?.owner_email);
    const privileged = currentUser === owner || isVpQuality || isAdministrator;
    if (!privileged) return alert("Only the NCMR owner, Administrator, or VP Quality can reassign this Rework task from the NCMR.");
    const nextEmail = window.prompt("Enter the new Rework Owner email:", task?.assigned_to_email || "");
    if (!nextEmail?.trim()) return;
    const reason = window.prompt("Reassignment reason is required:", "");
    if (!reason?.trim()) return alert("Reassignment reason is required.");
    const { error } = await supabase.from("approval_tasks").update({ assigned_to_email: nextEmail.trim().toLowerCase(), status: "pending", returned_reason: null, returned_by: null, returned_at: null }).eq("id", task.id).eq("entity_id", id).eq("task_type", "rework_task");
    if (error) return alert(error.message);
    await addAuditLog("rework_task_reassigned", `Rework task reassigned from ${task?.assigned_to_email || "N/A"} to ${nextEmail.trim().toLowerCase()} by ${userEmail}. Reason: ${reason.trim()}`);
    await createInAppNotification({ recipientEmail: nextEmail.trim().toLowerCase(), notificationType: "ncmr_rework_reassigned", title: `Rework task assigned: ${record?.ncmr_number || "NCMR"}`, message: `A Rework task was reassigned to you. Reason: ${reason.trim()}`, severityLevel: "info", assignedRole: "Rework Owner", relatedUrl: `/ncmrs/${id}/rework?taskId=${task.id}` });
    await fetchReworkTasks();
  };

  const reviseAndResubmitReturnedReworkTask = async (task: any) => {
    if (record?.is_locked) return alert("Closed NCMRs cannot resubmit Rework tasks.");
    const currentUser = normalizeApproverEmail(userEmail);
    const owner = normalizeApproverEmail(record?.owner || record?.owner_email);
    if (currentUser !== owner && !isVpQuality && !isAdministrator) return alert("Only the NCMR owner, Administrator, or VP Quality can revise and resubmit a returned Rework task.");
    const instructions = window.prompt("Revise the Rework Task as needed:", task?.task_instructions || task?.comments || "");
    if (!instructions?.trim()) return;
    const assignee = window.prompt("Rework Owner email:", task?.assigned_to_email || "");
    if (!assignee?.trim()) return;
    const dueDate = window.prompt("Due date (YYYY-MM-DD), or leave unchanged:", task?.due_date || "");
    const { error } = await supabase.from("approval_tasks").update({ task_instructions: instructions.trim(), comments: instructions.trim(), assigned_to_email: assignee.trim().toLowerCase(), due_date: dueDate?.trim() || task?.due_date || null, status: "pending" }).eq("id", task.id).eq("entity_id", id).eq("task_type", "rework_task").eq("status", "returned");
    if (error) return alert(error.message);
    await addAuditLog("rework_task_resubmitted", `Returned Rework task revised and resubmitted by ${userEmail} to ${assignee.trim().toLowerCase()}. Prior return reason preserved: ${task?.returned_reason || "N/A"}`);
    await createInAppNotification({ recipientEmail: assignee.trim().toLowerCase(), notificationType: "ncmr_rework_resubmitted", title: `Rework task resubmitted: ${record?.ncmr_number || "NCMR"}`, message: `The returned Rework task was revised and resubmitted. Open the task from My Workspace.`, severityLevel: "info", assignedRole: "Rework Owner", relatedUrl: `/ncmrs/${id}/rework?taskId=${task.id}` });
    await fetchReworkTasks();
  };
      ...
      >
        ...
        {hasReworkDisposition() && record.mrb_approved_by ? (
          ...
          >
            ...
            {!hasActiveOrCompletedReworkTask() ? (
              <>
                ...
                <div style={{ marginTop: "10px" }}>
                  <label>Rework Task Attachment (Optional)</label><br />
                  <input type="file" disabled={isLocked} onChange={(e) => setReworkAssignmentFile(e.target.files?.[0] || null)} />
                  <div style={{ color: "#64748b", fontSize: "13px", marginTop: "4px" }}>
                    Optional: attach an approved rework instruction, traveler, protocol, drawing, or other document supplied to the Rework Owner for execution.
                  </div>
                </div>
          ...
          <h4>Rework Task Status</h4>
            ...
            {reworkTasks.filter((task: any) => ["pending", "returned"].includes(String(task?.status || "").toLowerCase())).map((task: any) => (
              <div key={`manage-${task.id}`} style={{ marginTop: "8px", border: "1px solid #cbd5e1", borderRadius: "8px", padding: "10px", background: "#fff" }}>
                <strong>{String(task?.status || "").toLowerCase() === "returned" ? "Returned Rework Task" : "Active Rework Task"}</strong>
                {task?.returned_reason ? <div style={{ marginTop: "5px", color: "#991b1b" }}><strong>Return Reason:</strong> {task.returned_reason}</div> : null}
                <div style={{ marginTop: "8px" }}>
                  <button type="button" onClick={() => reassignReworkTaskFromNcmr(task)} disabled={isLocked}>Reassign Rework Task</button>
                  {String(task?.status || "").toLowerCase() === "returned" ? <button type="button" onClick={() => reviseAndResubmitReturnedReworkTask(task)} disabled={isLocked} style={{ marginLeft: "8px" }}>Revise & Resubmit Rework Task</button> : null}
                </div>
              </div>
            ))}
            ...
            {hasCompletedReworkTask() ? (
              <div style={{ marginTop: "18px", border: "1px solid #86efac", borderRadius: "8px", padding: "12px", background: "#f0fdf4" }}>
                ...
                <p style={{ color: "#166534", fontSize: "14px" }}>
                  Review the assigned Rework task, assignment attachment, task completion comment, objective evidence, attachments, final disposition, and reconciled final quantities before independently verifying the Rework.
                </p>
                <div style={{ display: "grid", gap: "12px" }}>
...
function ReworkOwnerVerificationCard({
  ...
  return (
    <div style={{ border: isVerified ? "1px solid #86efac" : "1px solid #93c5fd", background: isVerified ? "#f0fdf4" : "#fff", borderRadius: "10px", padding: "14px" }}>
      <div style={{ marginBottom: "12px" }}>
        <strong>Assigned Rework Task</strong>
        ...
        {Array.isArray(task?.assignment_attachments) && task.assignment_attachments.length > 0 ? (
          <div style={{ display: "grid", gap: "6px", marginTop: "7px" }}>
            {task.assignment_attachments.map((attachment: any, index: number) => (
              <a key={`${attachment?.storage_path || attachment?.url || index}`} href={attachment?.url} target="_blank" rel="noreferrer">📎 {attachment?.name || `Assignment Attachment ${index + 1}`}</a>
            ))}
          </div>
        ) : <div style={{ marginTop: "5px", color: "#64748b" }}>No optional assignment attachment was provided.</div>}
      </div>
