...
export default function ChangeControlWorkflowPage() {
  ...
  const fetchData = async () => {
    ...
    const [
      ...
      supabase
        ...
        .select("id, template_name, module_name, active")
  ...
  const activeDocuments = documents.filter((doc) => !doc.cancelled);

  const linkedControlledDocuments = activeDocuments
    .map((doc) =>
      controlledDocuments.find((cd) => cd.id === doc.controlled_document_id),
    )
    .filter(Boolean);
  ...
  const documentsReleased =
    activeDocuments.length === 0
      ...
      : activeDocuments.every((doc) => {
          const linkedDoc = controlledDocuments.find(
            (cd) => cd.id === doc.controlled_document_id,
          );
          return Boolean(
            linkedDoc &&
            (linkedDoc.status === "release" ||
              linkedDoc.status === "superseded"),
          );
        });
  ...
  const trainingComplete =
    ...
    activeDocuments.every((doc) => {
      ...
      const docTraining = trainingAssignments.filter(
        (item) => item.document_id === linkedDoc.id,
      );
      return (
        docTraining.length > 0 &&
        docTraining.every(
          (item) =>
  ...
  const hasRequiredReviewers = reviewers.some(
    (reviewer) => reviewer.required_reviewer,
  );

  const productsDispositioned =
    products.length === 0
      ? true
      : products.every(
          (product) =>
  ...
  const savePlanningPackage = async () => {
    ...
    if (missingImpactSummaries.length > 0) {
      return alert(
        `Impact summary is required for: ${missingImpactSummaries.join(", ")}.`,
      );
    }

    if (!riskForm.risk_review_summary.trim())
      return alert("Risk review summary is required.");
    if (!riskForm.risk_acceptability.trim())
      return alert("Risk acceptability is required.");
  ...
  const loadApprovalTemplate = async () => {
    ...
    if (!selectedTemplateId)
      return alert("Select an approval matrix template.");
    ...
    if (rows.length === 0)
      return alert("The selected template has no active reviewer rows.");
  ...
  const reviewerDecision = async (
    ...
    if (decision === "rejected" && !comments.trim())
      return alert("Rejection comments are required.");
    ...
    await createESignature({
      ...
      actionType:
        decision === "approved"
          ? "approve_change_reviewer"
          : "reject_change_reviewer",
      ...
      signatureReason:
        comments ||
        (decision === "approved"
          ? "Approved change control review."
    ...
    if (decision === "rejected") {
      await supabase
        ...
        .update({
          status: "rejected",
          approval_comments: comments || "Rejected during change review.",
          updated_at: new Date().toISOString(),
        })
        .eq("id", change.id);
    }
  ...
  const updateStatus = async (status: string) => {
    ...
    if (status === "pending_approval") {
      ...
      if (
        !riskForm.risk_review_summary.trim() ||
        !riskForm.risk_acceptability.trim()
      ) {
        return alert(
    ...
    if (status === "approved") {
      if (!canApprove)
        return alert("Only authorized approvers/admin/VP Quality can approve.");
      if (!requiredReviewersApproved)
        return alert(
          "All required reviewers must approve before the change can be approved.",
        );
      payload.approved_at = new Date().toISOString();
      payload.approved_by = userEmail;
      payload.approval_comments =
        "Change approved after approval matrix review.";
    }
    ...
    if (status === "verification") {
      if (!implementationComplete)
        return alert(
          "All implementation tasks must be complete before verification.",
        );
    }
  ...
  const resubmitRejectedChange = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        status: "draft",
        approval_comments: rejectionComment,
        updated_at: new Date().toISOString(),
      })
      .eq("id", change.id);
  ...
  const addDocument = async () => {
    ...
    if (!canImplement)
      return alert(
        "Affected document implementation is available after change approval.",
      );
    if (!newDoc.document_number.trim())
      return alert("Document number is required.");
  ...
  const cancelPlannedDocument = async (linkedDoc: any) => {
    if (!canImplement && !canVerify) {
      return alert(
        "Planned documents can only be cancelled after change approval and before closure.",
      );
    }

    if (linkedDoc.controlled_document_id) {
      return alert(
        "A linked document workflow already exists. Do not cancel it from Change Control; manage it through Document Control.",
      );
    }

    const reason = window.prompt(
      "Enter cancellation reason for this planned document:",
    );
    if (!reason || !reason.trim()) {
  ...
  const createControlledDocumentFromChange = async (linkedDoc: any) => {
    ...
    if (!canImplement)
      return alert(
        "Controlled document revisions can only be created after change approval.",
      );
    ...
    const matchingDocuments = controlledDocuments
      .filter(
        (doc: any) =>
          String(doc.document_number || "").trim() === documentNumber,
      )
      .sort((a: any, b: any) =>
        String(b.created_at || "").localeCompare(String(a.created_at || "")),
      );

    const releasedOrHistoricalDocuments = matchingDocuments.filter(
      (doc: any) =>
        doc.status === "release" ||
    ...
    const sourceDoc = currentRevision
      ? matchingDocuments.find(
          (doc: any) => String(doc.revision || "").trim() === currentRevision,
        ) ||
    ...
    const insertPayload: any = {
      ...
      document_type:
        sourceDoc?.document_type || masterSourceDoc?.document_type || null,
      department: sourceDoc?.department || masterSourceDoc?.department || null,
      process_area:
        sourceDoc?.process_area || masterSourceDoc?.process_area || null,
      ...
      revision_change_description:
        linkedDoc.change_description || change.change_description,
      revision_change_justification: change.change_justification,
      superseded_document_id: sourceDoc?.id || null,
      superseded_by_document_id: null,
      owner_email:
        change.owner_email ||
        sourceDoc?.owner_email ||
        masterSourceDoc?.owner_email ||
        userEmail ||
        null,
      approver_email: null,
      effective_date: null,
      ...
      training_impact: linkedDoc.training_required
        ? "FORMAL_TRAINING"
        : "NO_TRAINING",
    ...
    const linkRes = await supabase
      ...
      .update({
        ...
        current_revision:
          sourceDoc?.revision || linkedDoc.current_revision || null,
        proposed_revision: revisionToCreate,
        document_title:
          linkedDoc.document_title || sourceDoc?.title || documentNumber,
  ...
  const addProduct = async () => {
    if (!canImplement)
      return alert("Affected products are managed after change approval.");
    if (!newProduct.product_part_number.trim())
      return alert("Product part number is required.");
  ...
  const addTask = async () => {
    if (!canImplement)
      return alert("Implementation tasks are managed after change approval.");
    if (!newTask.task_title.trim()) return alert("Task title is required.");
    if (!normalizeEmail(newTask.owner_email))
      return alert("Task owner email is required.");
  ...
  const saveProductDisposition = async (product: any) => {
    ...
    const form = productDispositionForms[product.id] || {
      ...
      product_disposition_rationale:
        product.product_disposition_rationale || "",
      product_disposition_owner_email:
        product.product_disposition_owner_email || userEmail || "",
      product_disposition_date:
        product.product_disposition_date ||
    ...
    if (
      form.product_disposition_owner_email &&
      !normalizeEmail(form.product_disposition_owner_email)
    ) {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        product_disposition_rationale:
          form.product_disposition_rationale.trim(),
        product_disposition_owner_email:
          normalizeEmail(form.product_disposition_owner_email) ||
          userEmail ||
          null,
        product_disposition_date:
          form.product_disposition_date ||
          new Date().toISOString().slice(0, 10),
  ...
  const completeTask = async (task: any) => {
    ...
    if (!form.completion_summary.trim()) {
      return alert(
        "Completion summary is required before completing the task.",
      );
    }
  ...
  if (!change)
    return (
      <main style={pageStyle}>
        ...
        <a href="/change-control">Back to Change Control</a>
      </main>
    );
    ...
    <main style={pageStyle}>
      <header style={headerStyle}>
        <div>
          ...
          <h1 style={{ margin: "6px 0" }}>
            {change.change_number || "Change"} — {change.change_title}
          </h1>
      ...
      <section style={cardStyle}>
        ...
        <div style={buttonRowStyle}>
          {change.status === "draft" || change.status === "rejected" ? (
            <button
              onClick={savePlanningPackage}
              disabled={busy}
              style={primaryButtonStyle}
            >
              Save Planning Package
            </button>
          ) : null}
          {change.status === "draft" || change.status === "rejected" ? (
            <button
      ...
      <section style={cardStyle}>
        ...
        {canEditPlanning ? (
          <>
            <div style={gridStyle}>
              <Field label="Change Title">
                <input
                  value={initiationForm.change_title}
                  onChange={(e) =>
                    setInitiationForm({
                      ...initiationForm,
              ...
              <Field label="Change Type">
                <select
                  value={initiationForm.change_type}
                  onChange={(e) =>
                    setInitiationForm({
                      ...initiationForm,
              ...
              <Field label="Change Category">
                <select
                  value={initiationForm.change_category}
                  onChange={(e) =>
                    setInitiationForm({
                      ...initiationForm,
            ...
            <Field label="Change Description">
              <textarea
                value={initiationForm.change_description}
                onChange={(e) =>
                  setInitiationForm({
                    ...initiationForm,
            ...
            <Field label="Change Justification / Rationale">
              <textarea
                value={initiationForm.change_justification}
                onChange={(e) =>
                  setInitiationForm({
                    ...initiationForm,
                    change_justification: e.target.value,
                  })
                }
                rows={4}
                style={textareaStyle}
          ...
          <div style={gridStyle}>
            ...
            <Detail
              label="Justification"
              value={change.change_justification || "N/A"}
            />
          </div>
        )}
      </section>
        ...
        {canEditPlanning ? (
            ...
            <div style={impactGridStyle}>
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    product_impact: value,
                    product_impact_summary: value
                      ? assessmentForm.product_impact_summary
                      : "",
                  })
                }
              ...
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    document_impact: value,
                    document_impact_summary: value
                      ? assessmentForm.document_impact_summary
                      : "",
                  })
                }
                onSummaryChange={(value) =>
              ...
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    process_impact: value,
                    process_impact_summary: value
                      ? assessmentForm.process_impact_summary
                      : "",
                  })
                }
                onSummaryChange={(value) =>
              ...
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    equipment_impact: value,
                    equipment_impact_summary: value
                      ? assessmentForm.equipment_impact_summary
                      : "",
                  })
                }
                onSummaryChange={(value) =>
              ...
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    supplier_impact: value,
                    supplier_impact_summary: value
                      ? assessmentForm.supplier_impact_summary
                      : "",
                  })
                }
                onSummaryChange={(value) =>
              ...
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    software_impact: value,
                    software_impact_summary: value
                      ? assessmentForm.software_impact_summary
                      : "",
                  })
                }
                onSummaryChange={(value) =>
              ...
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    regulatory_impact: value,
                    regulatory_impact_summary: value
                      ? assessmentForm.regulatory_impact_summary
                      : "",
                  })
                }
                onSummaryChange={(value) =>
              ...
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    validation_impact: value,
                    validation_impact_summary: value
                      ? assessmentForm.validation_impact_summary
                      : "",
                  })
                }
                onSummaryChange={(value) =>
              ...
              <ImpactAssessmentEditor
                ...
                onCheckedChange={(value) =>
                  setAssessmentForm({
                    ...assessmentForm,
                    training_impact: value,
                    training_impact_summary: value
                      ? assessmentForm.training_impact_summary
                      : "",
                  })
                }
                onSummaryChange={(value) =>
          ...
          <>
            <div style={impactGridStyle}>
              <Impact
                label="Product"
                value={change.product_impact}
                summary={change.product_impact_summary}
              />
              <Impact
                label="Document"
                value={change.document_impact}
                summary={change.document_impact_summary}
              />
              <Impact
                label="Process"
                value={change.process_impact}
                summary={change.process_impact_summary}
              />
              <Impact
                label="Equipment"
                value={change.equipment_impact}
                summary={change.equipment_impact_summary}
              />
              <Impact
                label="Supplier"
                value={change.supplier_impact}
                summary={change.supplier_impact_summary}
              />
              <Impact
                label="Software"
                value={change.software_impact}
                summary={change.software_impact_summary}
              />
              <Impact
                label="Regulatory"
                value={change.regulatory_impact}
                summary={change.regulatory_impact_summary}
              />
              <Impact
                label="Validation"
                value={change.validation_impact}
                summary={change.validation_impact_summary}
              />
              <Impact
                label="Training"
                value={change.training_impact}
                summary={change.training_impact_summary}
              />
            </div>
          </>
        )}
        ...
        {canEditPlanning ? (
          <>
            <div style={gridStyle}>
              <Field label="Risk Level">
                <select
                  ...
                  onChange={(e) =>
                    setRiskForm({ ...riskForm, risk_level: e.target.value })
                  }
                  style={inputStyle}
                >
                  {RISKS.map((x) => (
              ...
              <Field label="Risk Acceptability">
                <input
                  value={riskForm.risk_acceptability}
                  onChange={(e) =>
                    setRiskForm({
                      ...riskForm,
                      risk_acceptability: e.target.value,
                    })
                  }
                  style={inputStyle}
                />
              ...
              <Field label="Residual Risk">
                <input
                  value={riskForm.residual_risk}
                  onChange={(e) =>
                    setRiskForm({ ...riskForm, residual_risk: e.target.value })
                  }
                  style={inputStyle}
                />
              </Field>
            ...
            <Field label="Risk Review Summary">
              <textarea
                value={riskForm.risk_review_summary}
                onChange={(e) =>
                  setRiskForm({
                    ...riskForm,
                    risk_review_summary: e.target.value,
                  })
                }
                rows={4}
                style={textareaStyle}
          ...
          <div style={gridStyle}>
            ...
            <Detail
              ...
              value={change.risk_acceptability || "N/A"}
            />
            <Detail
              label="Residual Risk"
              value={change.residual_risk || "N/A"}
            />
            <Detail
              label="Risk Review Summary"
              value={change.risk_review_summary || "N/A"}
            />
          </div>
        )}
      </section>
      ...
      <section style={cardStyle}>
                ...
                <label>
                  <input
                    ...
                    onChange={(e) =>
                      setNewReviewer({
                        ...newReviewer,
                        required_reviewer: e.target.checked,
                      })
                    }
                  />{" "}
                  Required Reviewer
        ...
        {change.status === "rejected" ? (
          <div style={warningStyle}>
            ...
            <Field label="Resubmission Comment">
              <textarea
            ...
            <button onClick={resubmitRejectedChange} style={primaryButtonStyle}>
              Return to Draft for Update
            </button>
          </div>
        ) : null}
      </section>
      ...
      change.status === "closed" ? (
              ...
              <>
                <div style={gridStyle}>
                  ...
                  <input
                    ...
                    value={newDoc.document_title}
                    onChange={(e) =>
                      setNewDoc({ ...newDoc, document_title: e.target.value })
                    }
                    style={inputStyle}
                  />
                  <input
                    placeholder="Current Revision Optional"
                    value={newDoc.current_revision}
                    onChange={(e) =>
                      setNewDoc({ ...newDoc, current_revision: e.target.value })
                    }
                    style={inputStyle}
                  />
                  <input
                    placeholder="Proposed Revision Optional / Auto"
                    value={newDoc.proposed_revision}
                    onChange={(e) =>
                      setNewDoc({
                        ...newDoc,
                        proposed_revision: e.target.value,
                      })
                    }
                    style={inputStyle}
                  />
                ...
                <textarea
                  placeholder="Document change description"
                  value={newDoc.change_description}
                  onChange={(e) =>
                    setNewDoc({ ...newDoc, change_description: e.target.value })
                  }
                  rows={3}
                  style={textareaStyle}
                />
                <label>
                  <input
                    type="checkbox"
                    checked={newDoc.training_required}
                    onChange={(e) =>
                      setNewDoc({
                        ...newDoc,
                ...
                <div style={{ marginTop: "10px" }}>
                  <button onClick={addDocument} style={primaryButtonStyle}>
                    Add Document to Change
                  </button>
                </div>
              </>
            ) : null}
            <ul>
              {documents.map((doc) => {
                ...
                const isCancelled = Boolean(
                  doc.cancelled || doc.document_status === "cancelled",
                );
                return (
                  <li
                    key={doc.id}
                  ...
                  >
                    <strong>{doc.document_number}</strong> Rev{" "}
                    {doc.current_revision || "N/A"} →{" "}
                    {doc.proposed_revision || "N/A"}
                    ...
                    <div style={smallTextStyle}>
                      Training Required: {doc.training_required ? "Yes" : "No"}
                    </div>
                    {isCancelled ? (
                      <div style={warningStyle}>
                        <strong>Planned document cancelled.</strong>
                        <div>Reason: {doc.cancellation_reason || "N/A"}</div>
                        <div style={smallTextStyle}>
                          Cancelled by {doc.cancelled_by || "N/A"} on{" "}
                      ...
                      <div style={buttonRowStyle}>
                        <button
                          onClick={() =>
                            createControlledDocumentFromChange(doc)
                          }
                          style={primaryButtonStyle}
                        >
                          Create Controlled Document Revision
                        </button>
                        <button
                          onClick={() => cancelPlannedDocument(doc)}
                          style={dangerButtonStyle}
                        >
                          Cancel Planned Document
                        </button>
                      </div>
                    ) : linkedDoc ? (
                      <a
            ...
            {canImplement ? (
              <>
                <div style={gridStyle}>
                  <input
                    ...
                    onChange={(e) =>
                      setNewProduct({
                        ...newProduct,
                        product_part_number: e.target.value,
                      })
                    }
                    style={inputStyle}
                  />
                  ...
                  />
                  <input
                    placeholder="Lot / Serial Scope"
                    value={newProduct.lot_or_serial_scope}
                    onChange={(e) =>
                      setNewProduct({
                        ...newProduct,
                        lot_or_serial_scope: e.target.value,
                      })
                    }
                    style={inputStyle}
                  />
                ...
                <textarea
                  ...
                  value={newProduct.impact_description}
                  onChange={(e) =>
                    setNewProduct({
                      ...newProduct,
              ...
              {products.map((product) => {
                ...
                return (
                  <li key={product.id} style={listCardStyle}>
                    <strong>{product.product_part_number}</strong> —{" "}
                    {product.product_name || "N/A"}
                    <div style={smallTextStyle}>
                      Lot / Serial Scope: {product.lot_or_serial_scope || "N/A"}
                    </div>
                    <div style={smallTextStyle}>
                      Impact: {product.impact_description || "N/A"}
                    </div>
                    <div style={smallTextStyle}>
                      Disposition Status:{" "}
                      {dispositionComplete
                    ...
                    {dispositionComplete ? (
                      <div style={dispositionSummaryStyle}>
                        ...
                        {product.product_disposition || "N/A"}
                        <div>
                          <strong>Rationale:</strong>{" "}
                          {product.product_disposition_rationale || "N/A"}
                        </div>
                        <div>
                          <strong>Owner:</strong>{" "}
                          {product.product_disposition_owner_email || "N/A"}
                        </div>
                        <div>
                          <strong>Date:</strong>{" "}
                          {product.product_disposition_date || "N/A"}
                        </div>
                        <div style={smallTextStyle}>
                          Dispositioned by{" "}
                          {product.product_dispositioned_by || "N/A"}
                        </div>
                      </div>
                      ...
                      <div style={subCardStyle}>
                        ...
                        <div style={gridStyle}>
                          <Field label="Disposition">
                            <select
                              ...
                              onChange={(e) =>
                                setProductDispositionForms({
                                  ...productDispositionForms,
                                  [product.id]: {
                                    ...dispositionForm,
                                    product_disposition: e.target.value,
                                  },
                                })
                              }
                              style={inputStyle}
                          ...
                          <Field label="Disposition Owner Email">
                            <input
                              type="email"
                              value={
                                dispositionForm.product_disposition_owner_email
                              }
                              onChange={(e) =>
                                setProductDispositionForms({
                                  ...productDispositionForms,
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setProductDispositionForms({
                                  ...
                                  [product.id]: {
                                    ...dispositionForm,
                                    product_disposition_date: e.target.value,
                                  },
                                })
                              }
                              style={inputStyle}
                        ...
                        <Field label="Disposition Rationale">
                          <textarea
                            value={
                              dispositionForm.product_disposition_rationale
                            }
                            onChange={(e) =>
                              setProductDispositionForms({
                                ...productDispositionForms,
                                [product.id]: {
                                  ...dispositionForm,
                                  product_disposition_rationale: e.target.value,
                                },
                              })
                            }
                            rows={3}
                        ...
                        <button
                          onClick={() => saveProductDisposition(product)}
                          style={primaryButtonStyle}
                        >
                          Save Product Disposition
                        </button>
                      </div>
                    ) : null}
                  </li>
                  ...
                  >
                    ...
                    {taskCancelled ? (
                      ...
                      <div style={completionSummaryStyle}>
                        ...
                        <div style={smallTextStyle}>
                          Completed by {task.completed_by || "N/A"} on{" "}
                      ...
                      <div style={subCardStyle}>
                        ...
                        <Field label="Completion Summary">
                          <textarea
                            ...
                            onChange={(e) =>
                              setTaskCompletionForms({
                                ...taskCompletionForms,
                                [task.id]: {
                                  ...completionForm,
                                  completion_summary: e.target.value,
                                },
                              })
                            }
                            rows={3}
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setTaskCompletionForms({
                                ...
                                [task.id]: {
                                  ...completionForm,
                                  completion_evidence: e.target.value,
                                },
                              })
                            }
                            rows={2}
        ...
        <section style={cardStyle}>
          ...
          <p style={subtleText}>
            Affected documents, affected products, implementation tasks,
            verification, and closure are available after the change is
            approved.
          </p>
        </section>
      )}
...
function ImpactAssessmentEditor({
  ...
  return (
    <div style={impactEditorStyle}>
      <label style={{ fontWeight: 700 }}>{label} Impact</label>
...
const impactEditorStyle: React.CSSProperties = {
  background: "#f9fafb",
  border: "1px solid #e5e7eb",
  borderRadius: "12px",
  padding: "12px",
};
const yesNoRowStyle: React.CSSProperties = {
  display: "flex",
  gap: "16px",
  marginTop: "8px",
  marginBottom: "8px",
...
const dispositionSummaryStyle: React.CSSProperties = {
  background: "#ecfdf5",
...
const completionSummaryStyle: React.CSSProperties = {
  background: "#eff6ff",
